...
export const TOOL_DESCRIPTIONS: Record<ToolName, string> = {
  ...
  list_modules:
    "Discover approved company modules before writing or extending infrastructure. Each module includes source_url (underlying VCS repository), registry_source, source_kind, and versions. When registry_source is present, prefer it as the Terraform module source and pin a published version using version. Otherwise use the VCS source with a Git ref. Do not substitute the VCS URL for a linked registry module or invent a version when lookup fails; restore registry access first. Call get_module_details with the module ID and exact published version for registry-linked modules. For Git-only modules inspect the exact tag through connected VCS access before using its inputs and outputs. With persisted catalogs, last_synced_at reports freshness; syncing and stale results require background refresh. Do not repeatedly poll or invent missing versions.",
  list_vcs_connections:
