...
\`trigger_compliance_evaluation\` runs against the code already pushed to the workspace's linked branch - it has no access to your local working tree. Before calling it, \`git add\`/\`git commit\`/\`git push\` everything to the remote branch you intend to evaluate. Skipping this silently evaluates whatever was last pushed, not the changes you just made - the tool will not warn you, and the returned score will look valid while being stale. Note that a push often runs the evaluation for you: when CI compliance is enabled, pushing to the default branch or to a branch with an open pull/merge request auto-runs a FULL evaluation (all rules), the same CI check a PR shows. So after pushing, check once whether a run already started for your commit before triggering - if one did, use a scoped run (rule_ids/ruleset_id) only to re-verify the rules you fixed, and trigger a full run yourself only when no auto-run applies (a non-default branch with no open PR, or CI compliance off). Pass \`ref\` as the branch name you just pushed (e.g. \`main\`): that resolves the branch's current remote commit and records the branch on the run. Do NOT omit \`ref\` - omitting evaluates the workspace's current interactive checkout that can lag a fresh push - and do NOT pin a bare commit SHA - a SHA-scoped run records no branch and shows up as "branch unknown" in the product. The evaluation then runs in the background like a CI check: hand the user the results URL and move on. Do not sit in a sleep or poll loop waiting for it, and never report elapsed times or ETAs - you cannot measure them reliably, so the numbers will be wrong. Check the result once later or when the user asks.
